...
# Local CO2 emissions -----
# Total regional CO₂ emissions	4,390.8 kilotonnes	2020	-12%	BEIS
...
weca_las <- c("Bath and North East Somerset",
              "Bristol, City of",
...
# 
  ...
  (sector_emissions <- c("Domestic" = domestic,
                        "Transport" = transport,
...
read_filter_sheet <- function(sheet_name, weca_las, file_path) {
  readxl::read_xlsx(file_path, sheet = sheet_name, skip = 5) |> 
    ...
    filter(!str_detect(local_authority_code, "\\([A-Z]{2}\\)"),
...
cons_last_year <- str_match_all(consumption_file_path,
                              "([0-9]{4})") |> 
  pluck(1, 2) |> 
  as.integer()
...
no2_data_bristol_tbl |> 
  ...
  summarise(exc2021 = sum(ugm3_2021 >= 40.0, na.rm = TRUE),
