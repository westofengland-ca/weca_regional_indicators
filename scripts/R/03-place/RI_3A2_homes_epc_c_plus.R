...
#' @param cat_vec character vector of EPC categories to include in the count. Use "all" to include all categories.
...
RI_3A2_fact_tbl <- RI_3A2_in_cat_tbl |>
  ...
  mutate(
    period_start = as.Date(glue("{year_month}-01")),
